...
- 반응형
### 개발 기간
* 리팩토링 : 22.08.11 ~ 22.09.22(~ing)
* 초안 : 021.09.09 ~ 2021.11.23
...
#### 타입스크립트
- 타입 보장으로 잠재적인 버그를 방지하기 위함
- 타입에 따른 메서드 제공함으로 편리한 코딩
#### 리덕스 툴킷
- 기존 리덕스의 복잡하고 가독성 떨어지는 코드 개선
- 미들웨어 등을 쉽게 연결시키기 위함
#### 파이어베이스
...
2022.09.07 ~ 22.09.09
#### :mag_right: 문제점
똑같이 디비에 저장하는 기능의 함수를 디비 경로가 다르다는 이유로 분리하여 사용하였다.<br />
재사용성이 떨어진다는 판단이 들었다.
#### :bulb: 해결법
그래서 디비 경로나 디비에 저장할 데이터들을 공통적으로 인자로 받아<br />
함수를 통합시켰다.
...
2022.09.11 ~ 22.09.14
#### :mag_right: 문제점
타입스크립트로 변경 중에 함수의 리턴값이 다르지만 같은 함수를 재사용하는 게 있었다.<br />
리턴값이 달라 타입은 다르지만 타입 때문에 같은 기능을 하는 함수를 나누는 게 재사용성 측면에서는 좋아보이지 않았다.
#### :bulb: 해결법
인자값, 리턴값 등의 타입을 T로 받은 후 함수를 호출 시 타입을 지정해주는 제네릭 함수를 이용하여 해결하였다.

### 새로고침 후 로그인 상태 유지
#### :alarm_clock: 기간
2022.08.29
#### :mag_right: 문제점
기존에 파이어베이스를 제공되는 메서드를 활용하여 이 문제를 해결하였다<br />
useEffect로 수시로 이 메서드를 불러오기 때문에 퍼포먼스적으로 비효율적이라는 판단이 들었다.
#### :bulb: 해결법
그래서 redux-persist 를 활용하여<br />
로그인 정보를 세션스토리지에 넣을 수 있게 하여 새로고침 후 로그인 상태가 유지 되었다.
### 페이지 이탈 후에도 유저가 누른 좋아요 상태 유지
#### :alarm_clock: 기간
2021.11.14
#### :mag_right: 문제점
